...
bool cursorPos(RECT *rcWindow, BYTE *xValue, BYTE *yValue) {
  static int oldX = 0; /* Cursor old pos */
  static int oldY = 0; 
  POINT mousePos;  /* Used to get the mouse position */
  int xPos;        /* X and Y positions of the mouse adjusted */ 
  int yPos;        /* to the window */
...
*NAME:          cursorSave
*AUTHOR:        John Morrison
*CREATION DATE: 19/6/00
*LAST MODIFIED: 19/6/00
*PURPOSE:
*  Saves a copy of the current cursor. Should be called
*  on start up to save a copy of the system cursor
*
*ARGUMENTS:
*
*********************************************************/
void cursorSave(void) {
  saveCurs = CopyCursor(GetCursor());
}
...
void cursorSetPos(RECT rcWindow, BYTE xValue, BYTE yValue) {
  ...
  if (cursorInMainView == TRUE && yValue != 0 && xValue != 0) {
    xValue--;
    yValue--;
    zoomFactor = windowGetZoomFactor();
