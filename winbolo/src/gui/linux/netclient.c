...
void gameFinderProcessBroadcast(currentGames *cg, INFO_PACKET *info, struct in_addr *pack) {
  ...
  currentGamesAddItem(cg, address, info->gameid.serverport, mapName, version, (BYTE) info->num_players, (BYTE) info->free_bases, (BYTE) info->free_pills, mines, info->gametype, info->allow_AI, password);
}
