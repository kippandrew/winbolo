...
extern unsigned char *mapUp;
extern long mapUpLen;
int parse_form_encoded(llist* entries)
{
  ...
  /* now start parsing */
  while ((bytesread=getline(buffer,BUFSIZ)) != 0 && done == 0) {
    start = 1;
    ...
    *tempstr = '\0';
	...
	if (tempstr) {
      ...
      fp = fopen("/usr/lib/cgi-bin/Members/upload.txt", "a");
      fprintf(fp, "%s - %s\n", REMOTE_ADDR, tempstr);
      fclose(fp);
      window = list_insafter(entries,window,entry);
      numentries++;
    ...
    j = 0;
    while (done == 0 && (mapUpLen+BUFSIZ) < (32 * 1024)) {
